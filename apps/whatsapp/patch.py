...
def _clone_whatsapp(root_dir):
    ...
    try:
        ...
        # 3. תיקון הרשאות ו-Authorities (השדרוג המרכזי)
        # יעדכן כל הרשאה או ספק שאינו של אנדרואיד עצמה
        def prefix_replacer(match):
            attr_name = match.group(1) # android:name או android:authorities
            original_value = match.group(2)
            
            # אם הערך הוא שם החבילה הישן, החלף בחדש
            if original_value == old_pkg:
                return f'{attr_name}="{new_pkg}"'
                
            # אם מתחיל בשם הישן, החלף רק את הקידומת
            if original_value.startswith(old_pkg + "."):
                return f'{attr_name}="{new_pkg}{original_value[len(old_pkg):]}"'

            # אם לא מתחיל ב-android/com.android (סטנדרטי), הוסף קידומת
            ...
            # אם זו הרשאה סטנדרטית, אל תיגע
            return match.group(0)

        # הרצת ה-Regex על הרשאות וספקים
        content = re.sub(r'(android:name|android:authorities)="(?!' + re.escape(old_pkg) + r'://)([^"]+)"', prefix_replacer, content)

        # תיקון נוסף עבור Intent Actions שאינם סטנדרטיים
        ...
        print(f"    [+] Successfully cloned (MT Manager logic). Package changed to: {new_pkg}")
        return True
